...
c2catp <- function(n=NULL){
  if(is.null(n))
    n <- sample(15:25, size=1)
  stem <- "Consider the data below on blood types, hair colors, medals, or races. Using this data, you may calculate the frequencies, relative frequencies (in fractional, reduced, and/or decimal forms), percentages, and midpoints. Using this information, you can graph bar charts, pareto charts, and pie charts."
  data <- c2cat_randdg(n)
  list(stem=stem, data=data)
}
